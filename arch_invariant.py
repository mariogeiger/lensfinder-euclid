...
class CNN:
    ...
    def NN(self, x):
        ...
        x = nn.relu(nn.scaleandshift(nn.convolution(x, 8*4, w=4, input_repr='invariant'))) # 98
        ...
        x = nn.relu(nn.scaleandshift(nn.convolution(x, 8*8))) # 46
        ...
        x = nn.relu(nn.scaleandshift(nn.convolution(x, 8*16))) # 20
        ...
        x = nn.relu(nn.scaleandshift(nn.convolution(x, 8*32))) # 7
        x = tf.nn.dropout(x, self.tfkp)
        ...
        x = nn.relu(nn.scaleandshift(nn.convolution(x, 8*128, w=5)))
        ...
        x = nn.relu(nn.scaleandshift(nn.fullyconnected(x, 8*256)))
        x = tf.nn.dropout(x, self.tfkp)
        ...
        x = nn.scaleandshift(nn.fullyconnected(x, 1, output_repr='invariant'), input_repr='invariant')
        return x
